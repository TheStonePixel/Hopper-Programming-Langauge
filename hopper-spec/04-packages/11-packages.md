...
file and the implementation file:
...
The `contract` field MUST point to a `.hop` file declaring an `interface` block. The
...
The compiler checks at compile time that the implementation satisfies the interface. If
...
<name>/
  ...
  interfaces/          # exported interface definitions (executables only)
```
