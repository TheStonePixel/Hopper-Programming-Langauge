...
The Hopper module system provides encapsulation at the contract level, not the name level.

A library declares its public contract in an contract file:
...
```hopper
// src/BufferImpl.hop
class BufferImpl satisfies Buffer {
    address data
    int capacity
    int position
...
A consuming program receives a binding that resolves to `Buffer` contract and `BufferImpl` implementation. The program calls methods on a `Buffer` contract — it sees only what the contract declares. The `_compact` helper, the `data` field, and the `position` field are not visible to the consumer because they are not in the contract.

The contract file IS the access control mechanism. It is explicit — the library author writes it, and only what appears in it is the public API.
...
Hopper's contract system already prevents callers from depending on implementation details — they only see the contract, not the implementing class. The second scenario — large team enforced OOP — is not Hopper's target domain. Systems code in a trusted codebase does not benefit from compiler-enforced visibility between files in the same project.
...
### The contract is the right abstraction boundary

Access modifiers enforce a boundary at the class level. Hopper's interfaces enforce a boundary at the module-external-API level. These are different concepts. A class may have many methods, some public and some private. An contract declares exactly the methods that external callers are expected to use.

The contract-level boundary is more useful than the class-level boundary for library code: it lets the implementation be restructured freely as long as the contract is preserved. Access modifiers would protect internal method names without protecting the contract contract.
...
For library code where formal encapsulation is required, the contract file is authoritative. The underscore convention is for application code where the discipline is social rather than mechanical.
