...
> `::address`, `::value`, and `::size` are implemented in the current compiler (grammar productions `Identifier '::' 'address'`, `Identifier '::' 'value'`, `Identifier '::' 'size'`). They are not merely a design goal.
...
> A function pointer must be unambiguous at the point of definition. Overloading is supported for free functions (see §6.2.10), but an overloaded function name cannot be taken as a `callback` value without ambiguity — the compiler cannot select which overload to use.
...
**Not yet implemented.** `allocate` and `deallocate` are compiler-built-in expression/statement forms, not addressable free functions. They cannot be passed as `callback` values directly. Wrapping them in ordinary Hopper functions (`function myAlloc(int n) address { return allocate n }`) is the current workaround.
...
**Not yet implemented.** Passing `allocate`/`deallocate` directly as callback arguments does not compile today (same reason as above).
...
class Pool {
    ...
    constructor(int size,
        ...
        callback(int) address doAlloc = cast self.allocFn   // Not yet implemented — cast<T>(expr) form required
        self.region   = doAlloc(size)
    }

    destructor() {
        callback(address) doFree = cast self.freeFn         // Not yet implemented — cast<T>(expr) form required
        doFree(self.region)
    }
}
```

**Not yet implemented.** The `cast expr` (without `<T>`) syntax shown in the constructor and destructor above is not valid Hopper. The actual cast syntax is `cast<T>(expr)`. The `callback` variable cast-from-address pattern is a forward design goal; the current compiler handles callback variables declared with explicit types and assigned from function names (e.g., `callback(int) address doAlloc = myAllocFn`).
...
> Generic types (`Ref<T>`, `Slice<T>`) are a forward compiler goal. The destructor mechanism exists today. The `cast 0` in the example is also not valid current syntax; use `return null` or `return cast<address>(0)` instead.
